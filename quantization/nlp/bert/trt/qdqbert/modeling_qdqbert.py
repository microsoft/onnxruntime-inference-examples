...
class QDQBertEmbeddings(nn.Module):
    ...
    def forward(
        ...
        #print("DEBUG==============================================================")
        #print(hasattr(self, "token_type_ids"))
        #print(device)
        ...
        if self.position_embedding_type == "absolute":
            position_ids_expanded = position_ids.expand(input_shape)
            position_embeddings = self.position_embeddings(position_ids_expanded)
            embeddings += position_embeddings
        
        # out = inputs_embeds + position_embeddings + token_type_embeddings
        # embeddings = out
...
@add_start_docstrings(
    "The bare QDQ Bert Model transformer outputting raw hidden-states without any specific head on top.",
...
class QDQBertForPreTraining(QDQBertPreTrainedModel):
    ...
    def forward(
        ...
        Example::
            ...
            >>> from transformers import BertTokenizer, QDQBertForPreTraining
            >>> import torch
...
@add_start_docstrings(
    """QDQ Bert Model with a `language modeling` head on top for CLM fine-tuning. """, QDQBERT_START_DOCSTRING
)
...
@add_start_docstrings("""QDQ Bert Model with a `language modeling` head on top. """, QDQBERT_START_DOCSTRING)
...
@add_start_docstrings(
    """
    QDQ Bert Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for
