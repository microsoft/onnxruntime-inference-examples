...
void run_ort_qnn_ep(std::string backend, std::string input_path, std::string qnn_native_context_binary = "") {
  ...
  OrtEnv* env;
  CheckStatus(g_ort, g_ort->CreateEnv(ORT_LOGGING_LEVEL_VERBOSE, "test", &env)); // Can set to ORT_LOGGING_LEVEL_INFO or ORT_LOGGING_LEVEL_VERBOSE for more info
  ...
  if (!qnn_native_context_binary.empty()) {
    ...
    // If not specifid, OnnxRuntime QNN EP will generate the context binary file from QDQ model at the frst run and load from it next time
