...
def create_custom_operator():
    ...
    graph = onnx.helper.make_graph(
        [custom_op_node],
        'custom_opone_model',
        [onnx.helper.make_tensor_value_info(name, onnx.TensorProto.FLOAT, [3]) for name in input_names],
        [onnx.helper.make_tensor_value_info(name, onnx.TensorProto.FLOAT, [3]) for name in output_names],
    )
