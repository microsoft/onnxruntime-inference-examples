...
EXPORT_SYMBOL OrtStatus* CreateEpFactories(const char* /*registration_name*/, const OrtApiBase* ort_api_base,
  ...
  std::unique_ptr<OrtEpFactory> factory = std::make_unique<BasicPluginEpFactory>(*ort_api, *ep_api, *model_editor_api,
                                                                                 *default_logger);
...
EXPORT_SYMBOL OrtStatus* ReleaseEpFactory(OrtEpFactory* factory) {
  EP_API_IMPL_BEGIN
  delete static_cast<BasicPluginEpFactory*>(factory);
  return nullptr;
  EP_API_IMPL_END
}
