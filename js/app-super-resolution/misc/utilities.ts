...
/**
 *
 * Note: some utility functions contained in this file (helper functions for converting RGB pixel to YCbCr/
 * YCbCr to RGB, etc.) to be removed. (currently still consumed by web.)
 * With updated ort-extensions support for data pre/post processing, converter functions will not be required.
 *
 */
...
function pixelsRGBToYCbCr(red: number, green: number, blue: number, mode: string) {
  let result = 0
  if (mode == 'y') {
    result = (0.299 * red + 0.587 * green + 0.114 * blue) / 255
  }
  else if (mode == 'cb') {
    result = ((-0.168935) * red + (-0.331665) * green + 0.50059 * blue) + 128
  }
  else if (mode == 'cr') {
    result = ((0.499813 * red + (-0.418531) * green + (-0.081282) * blue) + 128)
  }
...
function pixelsYCbCrToRGB(pixel: number, cb: number, cr: number, platform: PlatformTypes) {
  ...
  const green = Math.min(Math.max((y + ((-0.34373) * (cb - 0x80)) + ((-0.7144) * (cr - 0x80))), 0), 255);
  ...
  if (platform == 'web') {
    ...
    const intPixel = Array.of(((0xFF << 24) | ((0xFF & blue) << 16) | ((0xFF & green) << 8) | ((0xFF & red))))
 ...
 * - Mode `YCbCr`: It produces the input array needed by the model. Converts the RGB pixel array gotten from the input
 * image to a float32 array containing Y' channel pixels
 *
 * - Mode `RGB`: It produces the output array needed by the mobile and web APIs to generate an output image. Converts
 * the model output float32 array containing Y' channel pixels to an RGBA pixel array
 */
 ...
 * It loads the model as an asset and the source is obtained. Then an inference session is created with the model in the
 * platform environment. Returns the model.
 */
export async function loadModelAll(ort: any) {
  // const assets = await Asset.loadAsync(require('../assets/super_resnet12.ort'));
  const assets = await Asset.loadAsync(require('../assets/supres.onnx'));
 ...
 * After the input tensor for the model is created, this function is called to run the model with the input data and
 * obtain a float32 output array. Returns the output array.
 */
export async function runModelAll(inputData: any, model: any) {
  ...
  if (!output) {
    ...
    const outputArray = output.data as Uint8Array
