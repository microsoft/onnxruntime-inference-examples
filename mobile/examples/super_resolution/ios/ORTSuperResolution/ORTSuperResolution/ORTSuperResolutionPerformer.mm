...
// TODO: add field Result to ortSuperResClass
...
+ (BOOL)performSuperResolution:(NSError **)error {
    ...
    try {
      ...
      NSString *input_image_path =
          [NSBundle.mainBundle pathForResource:@"test_superresolution" ofType:@"png"];
      if (input_image_path == nullptr) {
        throw std::runtime_error("Failed to get image path");
      }
     
      // Prepare input tensors and input/output names
     ...
     /* TODO: update output analysis
      ...
      // output raw byte array - access by output_data_raw[i]
      const uint8_t *output_data_raw = output_tensor.GetTensorData<uint8_t>();*/
   
      // TODO: Add display raw pixel array to the screen as image
